...
export function loadLocalData(fallback) {
  try {
    ...
    for (const key of LEGACY_KEYS) { const saved = localStorage.getItem(key); if (saved) { const migrated = normalize(JSON.parse(saved), fallback); localStorage.setItem(STORAGE_KEY, JSON.stringify(migrated)); return migrated } }
  } catch {}
  return fallback
}
