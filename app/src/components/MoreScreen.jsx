...
export default function MoreScreen({values,onToggle,user,onSignOut,language='ko',onLanguage,onBackup,onRestore,onReset,onSeed,onEmpty,onUnread,testMode=false,onExitTestMode}){
 ...
 const items=useMemo(()=>[
  ...
  {h:testMode?116:82,node:<div className="more-qsection"><p className="more-section-label">{t(language,'testTools')}</p>{testMode&&<button className="test-mode-banner" data-act="test-exit" onClick={onExitTestMode}>{language==='en'?'Screen test mode · Return to server data':'화면 테스트 모드 · 서버 데이터로 돌아가기'}</button>}<Row action="test-seed" icon={icons.starter} label={t(language,'seed')} tail="⟲" onClick={onSeed||onReset}/></div>},
 ...
 ],[values.notifications,user,language,onToggle,onLanguage,onBackup,onReset,onSeed,onEmpty,onUnread,onSignOut,testMode,onExitTestMode])
 ...
 useLayoutEffect(()=>{if(entryAligned.current||heights.length!==items.length||!stageRef.current)return undefined;entryAligned.current=true;const desired=Math.max(0,stageRef.current.clientHeight/2-10);let nearest=0,best=Infinity;slotPositions.forEach((position,index)=>{const distance=Math.abs(position-desired);if(distance<best){best=distance;nearest=index}});q.setIndex(nearest);const frame=requestAnimationFrame(()=>setEntrySettled(true));return()=>cancelAnimationFrame(frame)},[heights.length,items.length])
 ...
 useEffect(()=>{if(!entrySettled){lastQueueIndex.current=q.index;return}if(lastQueueIndex.current===null){lastQueueIndex.current=q.index;return}if(lastQueueIndex.current!==q.index){lastQueueIndex.current=q.index;if(!locked)spin()}},[q.index,locked,entrySettled])
