...
export default function MoreScreen({values,onToggle,user,onSignOut,language='ko',onLanguage,onBackup,onRestore,onReset,onSeed,onEmpty,onUnread,onStub}){
 const fileRef=useRef(null),trackRef=useRef(null),stageRef=useRef(null),entryAligned=useRef(false),lastQueueIndex=useRef(null),rollTimer=useRef(null),initial=useMemo(loadSlot,[]),[locked,setLocked]=useState(initial.locked),[symbols,setSymbols]=useState(initial.symbols),[rolling,setRolling]=useState(false),[history,setHistory]=useState(false),[heights,setHeights]=useState([])
