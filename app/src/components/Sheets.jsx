...
function LegacyCircleEditor({circle,profile,onSave,onInvite,onCopyCode,onReorder,onLeave,onClose,language='ko'}){
 const en=language==='en'
...
export function ConfirmDialog({title,message,confirmLabel,danger,onCancel,onConfirm,language='ko'}){return <div className="modalwrap on"><button className="scrim" aria-label={language==='en'?'Cancel':'취소'} data-act="modal-cancel" onClick={onCancel}/><div className="modal" role="alertdialog" aria-modal="true"><h3>{title}</h3><p className="confirm-message">{message}</p><div className="mrow"><button className="mbtn" data-act="modal-cancel" onClick={onCancel}>{language==='en'?'Cancel':'취소'}</button><button className={`mbtn ${danger?'danger':'primary'}`} data-act="modal-ok" onClick={onConfirm}>{confirmLabel||(language==='en'?'Confirm':'확인')}</button></div></div></div>}
