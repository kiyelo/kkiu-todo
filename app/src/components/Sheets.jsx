...
export function Sheet({title,subtitle,children,onClose,hideClose=false,language='ko',bodyRef,className=''}){
 ...
 return <div className="sheetwrap on" onPointerDown={event=>event.stopPropagation()}><button className="scrim" aria-label={language==='en'?'Close':'닫기'} data-act="sheet-close" onClick={closeFromScrim}/><section className={`sheet${className?` ${className}`:''}`} role="dialog" aria-modal="true" aria-labelledby={titleId}><div className="grab" aria-hidden="true"/><header className="sheethead"><b id={titleId}>{title}</b>{subtitle&&<div className="mini">{subtitle}</div>}{!hideClose&&<button className="sheetclose" data-act="sheet-close" onClick={onClose} aria-label={language==='en'?'Close':'닫기'}>×</button>}</header><div className="sheetbody" ref={setBodyRef}>{children}</div></section></div>
}
