...
function buildModel(active, filter, showContext, expanded, doneCount) {
  ...
  if (!filter) {
    active.forEach((task, globalIndex) => { slot(globalIndex); if (adBefore(globalIndex)) { rows.push({ kind: 'ad', globalIndex, y, key: `ad-${globalIndex}` }); y += H.ad; slot(globalIndex) } rows.push({ kind: 'task', task, globalIndex, y, key: task.id }); y += H.task })
...
export default function QueueScreen(props) {
  ...
  const visibleRows = useMemo(() => fullModel.rows.filter((row) => Math.abs(row.y - currentSlot.y) <= VIRTUAL_WINDOW_PX), [fullModel.rows, currentSlot.y])
  const model = { ...fullModel, rows: visibleRows }
  ...
  return <>{circle && <CircleFilters members={members} value={filter} onChange={onFilter} unread={circle.memberUnread} language={language} />}<div className={`stage q${queue.dragging ? ' dragging' : ''}${reorder ? ' reordering' : ''}${queue.edge ? ` edge-${queue.edge}` : ''}`} style={{ '--edge-pull': queue.edgeAmount, '--queue-center-shift': circle ? '-27px' : '0px' }} {...queue.gestureProps}><div className="qvp"><div className="qtrack" style={{ top: 'calc(50% + var(--queue-center-shift))', transform: `translate3d(0,calc(-${currentSlot.y}px + ${queue.dragY}px),0)` }}>{model.rows.map((row) => { const shift = row.y >= currentSlot.y ? 81 : 2; const style = { top: `${row.y + shift}px` }; if (row.kind === 'task') { let translate = 0; if (reorder) { if (row.task.id === reorder.id) translate = reorder.offset; else if (reorder.from < reorder.to && row.globalIndex > reorder.from && row.globalIndex <= reorder.to) translate = -H.task; else if (reorder.from > reorder.to && row.globalIndex >= reorder.to && row.globalIndex < reorder.from) translate = H.task } return <div className={`queue-task-row${row.task.id === draggingId ? ' reorder-dragging' : ''}`} data-task-id={row.task.id} data-task-index={row.globalIndex} key={row.key} style={{ ...style, top: `${row.y + (row.globalIndex >= currentSlot.globalIndex ? 81 : 2)}px`, transform: `translate3d(0,${translate}px,0)` }}>{card(row.task, liveIndex(row.globalIndex, row.task.id), { reorderable: !circle || !filter })}</div> } if (row.kind === 'ghost') { const who = members.find((member) => assignees(row.task).includes(member.id)); return <div className="ghostrow" key={row.key} style={style}><span className="grank">#{row.globalIndex + 1}</span><span className="gtitle">{row.task.title}</span>{who && <span className="who">{who.emoji}</span>}</div> } if (row.kind === 'bundle') return <button className="bundle" data-act="bundle" key={row.key} style={style} onClick={() => setExpanded(row.key)}>{language==='en'?<><b>{row.count}</b> more · #{row.from + 1}–#{row.to}</>:<><b>{row.count}개</b> 숨김 · #{row.from + 1}–#{row.to}</>}</button>; if (row.kind === 'collapse') return <button className="colbar" data-act="collapse" key={row.key} style={style} onClick={() => setExpanded(null)}>{language==='en'?`∧ Collapse (#${row.from + 1}–#${row.to})`:`#${row.from + 1}–#${row.to} 접기`}</button>; if (row.kind === 'ad') return <div aria-hidden={reorder ? 'true' : undefined} className="adcard" data-ad-anchor={row.globalIndex} key={row.key} style={style}><span className="adtag">AD</span><div className="adthumb" /><div className="admid"><p>{language === 'en' ? 'Your ad could be here' : '여기에 광고가 표시됩니다'}</p><p className="adsub">{language === 'en' ? 'In-feed native · excluded from ranks' : '인피드 네이티브 · 끼우 번호에서 제외'}</p></div></div>; return <button className="donebtn" data-act="sheet-open" key={row.key} style={style} onClick={onCompleted}>✓ {t(language, 'done', row.count)}</button> })}</div></div>{!active.length && <div className="queueempty"><div className="empty-c"><h3>{t(language, 'empty')}</h3></div></div>}<div className="queue-edge-feedback" aria-hidden="true" /><div className="qfade t" /><div className="qfade b" /><div className="queue-composer-wrap" aria-hidden={reorder ? 'true' : undefined} style={{ position: 'absolute', top: 'calc(50% + 30px + var(--queue-center-shift))', left: 0, right: 0 }}><Composer count={active.length} circle={circle} members={members} position={currentSlot.globalIndex} onAdd={(title, assignee, position, mode) => { const next = model.slots.findIndex((item) => item.globalIndex >= currentSlot.globalIndex + 1); if (next >= 0) queue.setIndex(next); onAdd(title, assignee, position, mode) }} onOpenChange={setComposerOpen} language={language} /></div></div></>
}
