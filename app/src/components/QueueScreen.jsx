...
export default function QueueScreen(props) {
  ...
  const [reorder, setReorder] = useState(null); const [assignOpen, setAssignOpen] = useState(false); const [composerOpen, setComposerOpen] = useState(false); const [expanded, setExpanded] = useState(null); const [flashId, setFlashId] = useState(focusTaskId)
  ...
  useEffect(() => { const next = model.slots.findIndex((item) => item.globalIndex >= wantedGlobal); if (next >= 0) queue.setIndex(next) }, [filter, composerOpen, expanded, focusTaskId, initialPosition])
  const draggingId = reorder?.id || null
  const startReorder = (id, event) => { const from = active.findIndex((task) => task.id === id); if (from >= 0) setReorder({ id, from, to: from, startY: event.clientY, offset: 0 }) }
  const dragMove = (event) => setReorder((current) => { if (!current) return current; const target = document.elementFromPoint(event.clientX, event.clientY)?.closest('[data-task-id]')?.dataset.taskId; const to = target ? active.findIndex((task) => task.id === target) : current.to; return { ...current, to: to >= 0 ? to : current.to, offset: event.clientY - current.startY } })
  const finishReorder = (_event, cancelled) => { if (reorder && !cancelled && reorder.to !== reorder.from) onMoveTo(reorder.id, active[reorder.to]?.id); setReorder(null) }
