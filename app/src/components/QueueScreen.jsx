...
export default function QueueScreen(props) {
  ...
  const startReorder = (id, event) => { const from = active.findIndex((task) => task.id === id); if (from >= 0) { const next = { id, from, to: from, startY: event.clientY, offset: 0 }; reorderRef.current = next; setReorder(next) } }
  const dragMove = (event) => { const current = reorderRef.current; if (!current) return; const rows = [...document.querySelectorAll('.queue-task-row[data-task-index]')].filter((row) => row.dataset.taskId !== current.id); let to = current.to; let best = Infinity; rows.forEach((row) => { const rect = row.getBoundingClientRect(), distance = Math.abs(event.clientY - (rect.top + rect.height / 2)); if (distance < best) { best = distance; to = Number(row.dataset.taskIndex) } }); const next = { ...current, to, offset: event.clientY - current.startY }; reorderRef.current = next; setReorder(next) }
