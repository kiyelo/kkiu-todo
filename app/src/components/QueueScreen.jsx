...
export default function QueueScreen({ tasks, members, circle, query, onQuery, filter, onFilter, onAdd, onComplete, onEdit, onAssignee, onMove, selecting, selected, onSelect, onLongPress, onSelectAll, onDeleteSelected, onCancelSelect }) {
  ...
  return (
    <>
      ...
      {selecting && <div className="selection-bar"><button onClick={onCancelSelect}>취소</button><b>{selected.size}개 선택</b><button onClick={onSelectAll}>전체</button><button className="danger" disabled={!selected.size} onClick={onDeleteSelected}>삭제</button></div>}
    </>
  )
}
