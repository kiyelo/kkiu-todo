...
export default function QueueScreen(props) {
  const { viewKey, focusVisit = 0, dataReady = true, tasks, members, circle, circleMode, onCreateCircle, onJoinCircle, query, onQuery, onSearchResult, focusTaskId, newTaskId, filter, onFilter, onAdd, onComplete, onEdit, onAssignee, onMove, onMoveTo, onDelete, onCompleted, initialPosition = null, onPositionChange, language = 'ko' } = props
  const [reorder, setReorder] = useState(null); const reorderRef = useRef(null); const reorderFrameRef = useRef(null); const reorderPointerYRef = useRef(null); const [composerOpen, setComposerOpen] = useState(false); const [expanded, setExpanded] = useState(null); const [flashId, setFlashId] = useState(focusTaskId)
  ...
  const card = (task, index, extra = {}) => <TaskCard key={extra.key || task.id} task={task} index={index} members={members} circle={circle} onComplete={onComplete} onEdit={onEdit} onAssignee={onAssignee} onMove={onMove} onDelete={onDelete} onDragStart={startReorder} onDragMove={dragMove} onDragEnd={finishReorder} dragging={draggingId === task.id} searchHit={flashId === task.id} newHit={newTaskId?.has?.(task.id) || false} language={language} {...extra} />
