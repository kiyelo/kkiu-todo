...
check('Android app theme preference reaches native bridge', androidThemePlugin.includes('setThemePreference') && themePlatform.includes('NativeTheme.setThemePreference'))
check('Android runtime theme bridge never writes native night mode', !androidThemePlugin.includes('setApplicationNightMode(') && !androidThemePlugin.includes('setDefaultNightMode('))
check('Android system theme read uses global UiModeManager state', androidThemePlugin.includes('manager.getNightMode()') && androidThemePlugin.includes('MODE_NIGHT_YES') && androidThemePlugin.includes('MODE_NIGHT_NO'))
check('Android system theme changes are received globally', androidThemePlugin.includes('Intent.ACTION_CONFIGURATION_CHANGED') && androidThemePlugin.includes('registerReceiver(configurationReceiver'))
