...
check('Android app theme preference reaches native theme bridge', androidThemePlugin.includes('setThemePreference') && themePlatform.includes('NativeTheme.setThemePreference'))
check('Android system preference is applied before effective theme is read', themePlatform.indexOf('await syncNativeThemePreference(normalized)') >= 0 && themePlatform.indexOf('await syncNativeThemePreference(normalized)') < themePlatform.indexOf("const resolvedTheme = normalized === 'system' ? await readSystemTheme() : normalized"))
...
check('System theme read uses global UiModeManager state', androidThemePlugin.includes('manager.getNightMode()') && androidThemePlugin.includes('MODE_NIGHT_YES') && androidThemePlugin.includes('MODE_NIGHT_NO'))
check('System theme read does not depend on Resources.getSystem', !androidThemePlugin.includes('Resources.getSystem()'))
