...
begin
  ...
  if initial_provider = 'email'
     and normalized_email not in (
       'qa-a@example.test',
       'qa-b@example.test',
       'qa-delete@example.test'
     ) then
...
comment on function private.reject_new_email_auth_users() is
  'Blocks new email Auth users except the fixed QA accounts and the disposable account-deletion QA account; social OAuth users remain allowed.';
